...
class HangmanAPI(object):
    def __init__(self, access_token=None, session=None, timeout=None):
        ...
        full_dictionary_location = "/kaggle/input/hangmandataset/words_250000_train.txt"
    ...
    def guess(self, word): # word input example: "_ p p _ e "
        ...
        if len(self.guessed_letters) > 0 and self.tries_remaining <= 5:
    ...
    def unigram_probs(self, word):
        ...
        return guess_letter
    def relative_guess(self,word):
        freq = {}
        for i in full_dictionary:
            if len(i) >= 2 and i[1] == word[2]:
                if freq.get(i[0]):
                    freq[i[0]] += 1
                else:
                    freq[i[0]] = 1
        max_key = max(freq, key=freq.get)
        if max_key not in self.guessed_letters:
            return max_key
    
    def Originalguess(self, word): # word input example: "_ p p _ e "
        ###############################################
    ...
    def start_game(self, practice=True, verbose=True):
        ...
        if response.get('status')=="approved":
            ...
            if tries_remains == 1 and word[0] == '_' and word[2] != '_':
                self.next_guess = self.relative_guess(word)
